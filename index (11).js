  ...
  handler: async (request, context) => {
    ...
    try {
      ...
      if (action === 'testUsage') {
        ...
        const startStr = startDate.toISOString().split('T')[0];
        const url = `https://api.powerbi.com/v1.0/myorg/admin/activityevents?startDateTime='${startStr}T00:00:00'&endDateTime='${endDate}T23:59:59'&$filter=Activity eq 'ViewReport'`;
        const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
        const text = await res.text();
        return { status: 200, headers: CORS_HEADERS, body: JSON.stringify({ httpStatus: res.status, response: text.slice(0, 2000) }) };
      }

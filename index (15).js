  ...
  handler: async (request, context) => {
    ...
    try {
      ...
      if (action === 'testReportPerms') {
        const token = await getToken();
        const wsData = await pbiGet(token, '/groups?$top=5');
        const results = [];
        for (const ws of (wsData.value || [])) {
          try {
            const rData = await pbiGet(token, `/groups/${ws.id}/reports`);
            const report = (rData.value || [])[0];
            if (!report) continue;
            const users = await pbiGet(token, `/groups/${ws.id}/reports/${report.id}/users`);
            results.push({ workspace: ws.name, report: report.name, userCount: (users.value || []).length, users: users.value || [] });
            if (results.length >= 3) break;
          } catch (e) { results.push({ workspace: ws.name, error: e.message }); }
        }
        // Test app permissions
        try {
          const appsData = await pbiGet(token, '/admin/apps?$top=5');
          const appResults = [];
          for (const app of (appsData.value || [])) {
            try {
              const appUsers = await pbiGet(token, `/admin/apps/${app.id}/users`);
              appResults.push({ app: app.name, userCount: (appUsers.value || []).length, sample: (appUsers.value || []).slice(0, 2) });
            } catch (e) { appResults.push({ app: app.name, error: e.message }); }
          }
          return { status: 200, headers: CORS_HEADERS, body: JSON.stringify({ reportPerms: results, appPerms: appResults }) };
        } catch (e) {
          return { status: 200, headers: CORS_HEADERS, body: JSON.stringify({ reportPerms: results, appPermsError: e.message }) };
        }
      }

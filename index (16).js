  ...
  handler: async (request, context) => {
    ...
    try {
      ...
      if (action === 'testReportPerms') {
        ...
        try {
          const wsData = await pbiGet(token, '/groups?$top=2');
          const ws = (wsData.value || [])[0];
          if (ws) {
            const rData = await pbiGet(token, `/groups/${ws.id}/reports`);
            const report = (rData.value || [])[0];
            if (report) {
              // Try both endpoints and capture raw status
              ...
              results.reportTest = {
                ...
                adminEndpoint: { status: r1.status, body: await r1.text() },
                standardEndpoint: { status: r2.status, body: await r2.text() }
              };
            }
          }

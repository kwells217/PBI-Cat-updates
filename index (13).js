...
async function fetchUsageMetrics(token) {
  try {
    const metrics = {};
    const now = new Date();

    // Pull 3 x 30-day windows to cover 90 days
    for (let i = 0; i < 3; i++) {
      ...
      try {
        let continuationUri = null;
        let pageCount = 0;
        do {
          const url = continuationUri ||
            `https://api.powerbi.com/v1.0/myorg/admin/activityevents?startDateTime='${startStr}T00:00:00'&endDateTime='${endStr}T23:59:59'&$filter=Activity eq 'ViewReport'`;
          const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
          if (!res.ok) break;
          const data = await res.json();
          (data.activityEventEntities || []).forEach(e => {
            const id = e.ReportId || e.reportId;
            if (!id) return;
            if (!metrics[id]) metrics[id] = { views: 0, viewers: new Set(), lastViewed: null };
            metrics[id].views++;
            if (e.UserId || e.userId) metrics[id].viewers.add(e.UserId || e.userId);
            const ts = e.CreationTime || e.creationTime;
            if (ts && (!metrics[id].lastViewed || new Date(ts) > new Date(metrics[id].lastViewed))) {
              metrics[id].lastViewed = ts;
            }
          });
          continuationUri = data.continuationUri || null;
          pageCount++;
        } while (continuationUri && pageCount < 20);
      } catch (e) { /* skip window */ }
    }

    return Object.entries(metrics).reduce((acc, [id, m]) => {
      acc[id] = { views: m.views, uniqueViewers: m.viewers.size, lastViewed: m.lastViewed };
      return acc;
    }, {});
  } catch (e) { return {}; }
}
...
async function buildFullCatalog(token) {
  ...
  // Fetch and attach usage metrics
  const usageMetrics = await fetchUsageMetrics(token);
  reports.forEach(r => {
    r._usage = usageMetrics[r.id] || { views: 0, uniqueViewers: 0, lastViewed: null };
  });
  ...
  handler: async (request, context) => {
    ...
    try {
      ...
      if (action === 'testUsage') {
        ...
        const startStr = startDate.toISOString().split('T')[0];
        const url = `https://api.powerbi.com/v1.0/myorg/admin/activityevents?startDateTime='${startStr}T00:00:00'&endDateTime='${endDate}T23:59:59'`;
        const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
        const text = await res.text();
        // Also test the audit log endpoint
        const url2 = `https://api.powerbi.com/v1.0/myorg/admin/auditlogs/directlyshareddashboards`;
        const res2 = await fetch(url2, { headers: { Authorization: `Bearer ${token}` } });
        const text2 = await res2.text();
        return { status: 200, headers: CORS_HEADERS, body: JSON.stringify({ activityEvents: { httpStatus: res.status, response: text.slice(0, 500) }, auditTest: { httpStatus: res2.status, response: text2.slice(0, 500) } }) };
      }

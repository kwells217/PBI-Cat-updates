  ...
  handler: async (request, context) => {
    ...
    try {
      ...
      if (action === 'testReportPerms') {
        const token = await getToken();
        const results = { reportTest: null, appTest: null, scanTest: null };

        // Test report users via admin endpoint
        try {
          ...
          if (report && ws) {
            const urls = [
              `https://api.powerbi.com/v1.0/myorg/admin/reports/${report.id}/users`,
              `https://api.powerbi.com/v1.0/myorg/groups/${ws.id}/reports/${report.id}/users`,
              `https://api.powerbi.com/v1.0/myorg/admin/groups/${ws.id}/reports/${report.id}/users`
            ];
            const testResults = [];
            for (const url of urls) {
              const r = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
              testResults.push({ url, status: r.status, body: (await r.text()).slice(0, 200) });
            }
            results.reportTest = { reportName: report.name, workspace: ws.name, endpoints: testResults };
          }
        } catch (e) { results.reportTest = { error: e.message }; }

        // Test app endpoint
        try {
          const appsUrl = `https://api.powerbi.com/v1.0/myorg/admin/apps?$top=2`;
          const r = await fetch(appsUrl, { headers: { Authorization: `Bearer ${token}` } });
          const body = await r.text();
          results.appTest = { status: r.status, body: body.slice(0, 500) };

          // If apps found, test app users
          if (r.status === 200) {
            const apps = JSON.parse(body);
            const app = (apps.value || [])[0];
            if (app) {
              const appUsersUrl = `https://api.powerbi.com/v1.0/myorg/admin/apps/${app.id}/users`;
              const r2 = await fetch(appUsersUrl, { headers: { Authorization: `Bearer ${token}` } });
              results.appUsersTest = { appName: app.name, status: r2.status, body: (await r2.text()).slice(0, 200) };
            }
          }
